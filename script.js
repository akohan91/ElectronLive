  ...
  constructor(plan) {
    plan.forEach((str, line) => {
      for (let col = 0; col < str.length; col++) {
        switch (str[col]) {
          case '#':
            this._wObjects.push(new Wall(new Place(line, col)));
            break;
          case 'o':
            this._wObjects.push(new PlantEating(new Place(line, col)));
            break;
          case '*':
            this._wObjects.push(new Plant(new Place(line, col)));
            break;
          default:
            break;
        }
